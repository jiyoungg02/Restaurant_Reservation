...
public class ReservationListDAO {
	...
	public List<ReservationDTO> getUsedReservations(String memberId) {
	    ...
	    try {
	        sql = "SELECT r.*, res.restaurant_name "
	            + "FROM Reservation r "
	            + "JOIN Restaurant res ON r.restaurant_id = res.restaurant_id "
	            + "WHERE r.is_used = 'Y' AND r.member_id = ? "
		   ...
		   List<ReservationDTO> list = new ArrayList<>();
		    ...
		    try {
		        sql = "SELECT r.*, res.restaurant_name "
		            + "FROM Reservation r "
		            + "JOIN Restaurant res ON r.restaurant_id = res.restaurant_id "
		            + "WHERE r.is_used = 'N' AND r.member_id = ? "
		            + "ORDER BY r.reservation_date, r.reservation_time";
	...
	// 예약 수정
	public void updateReservation(String reservation_id, Date reservation_date, String reservation_time, String number_of_people, String member_Id) throws SQLException {
	    PreparedStatement pstmt = null;
	    ResultSet rs = null;
	...
	public void updateExpiredReservations() {
	    ...
	    String sql = "UPDATE Reservation " +
	                 ...
	                 "OR (reservation_date = TRUNC(SYSDATE) " +
